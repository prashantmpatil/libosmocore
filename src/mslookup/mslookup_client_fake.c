					...
					uint32_t request_handle)
{
	...
	llist_for_each_entry(r, &state->requests, entry) {
		...
		llist_del(&r->entry);
		talloc_free(r);
		LOGP(DLMSLOOKUP, LOGL_DEBUG, "%s() ok\n", __func__);
		return;
	}
	LOGP(DLMSLOOKUP, LOGL_DEBUG, "%s() FAILED\n", __func__);
}
...
static void fake_lookup_async_response(void *data)
{
	...
	llist_for_each_entry_safe(req, n, &state->requests, entry) {
		...
		for (resp = state->responses;
			...
			LOGP(DLMSLOOKUP, LOGL_DEBUG, "osmo_mslookup_client_rx_result(): %s\n",
