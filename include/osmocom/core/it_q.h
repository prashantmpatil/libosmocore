 ...
 *  grabs a mutex for the actual list operations to safeguard againt races).  The
 *  receiving thread is woken up by an event_fd which can be registered in the libosmocore
 *  select loop handling.
 *
 *  IMPORTANT: Callers *MUST* ensure that items enqueued using osmo_it_q contain a
 *  'struct llist_head' at the very beginning. */
struct osmo_it_q {
	...
	void (*read_cb)(struct osmo_it_q *q, void *item);
